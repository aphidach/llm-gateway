...
rtk uv run python -m app.main
rtk pytest
```

- `rtk uv run python -m app.main`: start the API service (uses the configured FastAPI entrypoint).
- `rtk pytest`: run backend unit/integration tests.
...
- `rtk npm run dev`: start the frontend dev server.
- `rtk npm run build`: production build.
- `rtk npm run lint`: ESLint checks.
...
CI is not defined in this repo, so treat local checks as the gate: run `rtk pytest` for backend changes and `rtk npm run lint` (plus `rtk npm run build` when touching Next.js config) before opening a PR.
